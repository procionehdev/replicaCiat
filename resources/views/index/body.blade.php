<div class="w-full bg-primaryBackground px-4 md:pl-11">

    <div class="w-full h-[506px] md:h-[610px] md:px-0 md:py-0 mx-auto">
        <div class="md:grid md:grid-cols-5 md:grid-rows-1 bg-kartellSecondary md:mr-7">
            <div class="mx-auto w-full h-[176px] md:h-[610px] md:col-start-3 md:col-span-3 md:row-start-1 bg-center bg-cover bg-[url('{{ asset('assets/images/collectionPlaceholder1.jpg') }}')]">
            </div>
                <div class="px-4 pt-2 md:max-w-[450px] md:h-[310px] md:m-auto md:px-2 md:py-0 md:col-start-1 md:col-span-2 md:row-start-1">
                <p class="text-[12px] text-kartellMain font-din">MY KARTEL 2023</p>
                <p class="text-[28px] md:text-[46px] text-kartellMain font-switzer font-bold leading-9 md:leading-[60px] mt-1">Il nostro progetto per il futuro</p>
                <p class="text-[14px] text-kartellMain font-switzer font-light leading-5 mt-1.5 md:mt-4">La collezione 2023 di Kartell è un’espressione di design contemporaneo, caratterizzata da forme audaci, materiali innovativi e colori vivaci.</p>
                <button class="bg-kartellMain text-kartellSecondary mt-6 md:mt-8 h-[50px] w-[166px] font-switzer">Esplora la collezione</button>
            </div>
        </div>
    </div>
    ...
    <div class="carouselContainer">
        ...
        <div class="carouselScroll">
            @foreach($carousels[0]->products()->get()  as $product)
                <div class="w-[140px] md:w-[292px] h-[286px] md:h-[440px] mr-1.5 md:mr-4 shrink-0">
                    <div class="w-full h-[172px] md:h-[360px] flex bg-cover bg-center bg-[url('{{ asset($product->img) }}')]">
                        <img class="mr-[13px] md:mr-[22px] ml-auto mb-2 md:mb-7 mt-auto w-[25px] h-[25px]" src="{{ asset('assets/icons/heart.svg') }}">
                    </div>
                    <p class="text-[10px] font-din text-navText uppercase mt-2">{{  $product->designer }}</p>
                    <div class="md:flex mt-1">
                        <p class="text-[13px] md:text-[14px] font-switzer font-medium">{{  $product->name }}</p>
                        <p class="text-[14px] font-switzer font-light mt-2 md:mt-0 md:mr-0 md:ml-auto">{{  $product->price }} €</p>
                    </div>
                </div>
            @endforeach
        </div>
    ...
    <div class="carouselContainer">
        <div class="md:flex md:mx-auto md:ml-0">
            <div class="collectionPanel flex md:w-[600px] md:h-[420px] md:max-w-[600px] md:mr-4 bg-[url({{ asset('assets/images/flos.jpg') }})]">
            ...
            <div class="collectionPanel flex mt-2 md:mt-8 md:w-[908px] md:h-[420px] md:max-w-[908px] bg-[url({{ asset('assets/images/design.jpg') }})]">
                <div class="self-end pb-2 pl-1">
    ...
    <div class="carouselContainer">
        <div class="carouselGrid md:grid-rows-2 mb-4">
        ...
        <div class="md:grid md:grid-cols-6 md:grid-rows-1">
            <div class="mx-auto md:mx-0 w-[288px] h-[264px] md:col-start-1 md:col-span-2 md:w-full md:h-[550px]
            bg-cover bg-right bg-[url('{{asset('assets/images/estate.jpg')}}')]">
        </div>
            <div class="carouselScroll md:ml-4 md:col-start-3 md:col-span-4">
                @foreach($carousels[1]->products()->get()  as $product)
                    <div class="w-[214px] h-[353px] md:w-[446px] md:h-[610px] mr-1.5 shrink-0">
                        <div class="w-full h-[264px] md:h-[550px] flex bg-cover bg-center bg-[url('{{ asset($product->img) }}')]">
                            <img class="mr-[13px] md:mr-[22px] ml-auto mb-2 md:mb-7 mt-auto w-[25px] h-[25px]" src="{{ asset('assets/icons/heart.svg') }}">
                        </div>
                        <p class="text-[10px] font-din text-navText uppercase mt-2">{{  $product->designer }}</p>
                        <div class="md:flex mt-1">
                            <p class="text-[13px] md:text-[14px] font-switzer font-medium">{{  $product->name }}</p>
                            <p class="text-[14px] font-switzer font-light mt-2 md:mt-0 md:mr-1 md:ml-auto">{{  $product->price }} €</p>
                        </div>
                    </div>
                @endforeach
    ...
    <div class="carouselContainer">
        ...
        <div class="carouselScroll">
            @foreach($carousels[2]->products()->get()  as $product)
                <div class="w-[214px] h-[322px] md:w-[292px] md:h-[440px] mr-1.5 shrink-0">
                    <div class="w-full h-[322px] md:h-[440px] flex bg-cover bg-center bg-[url('{{ asset($product->img) }}')]">
                        <div class="w-full h-full bg-gradient-to-t from-gray-600 to-40% flex">
                            <div class="mb-4 ml-3 mt-auto">
                                <p class="text-[18px] md:text-[24px] text-white font-switzer font-medium">{{  $product->name }}</p>
                            </div>
                        </div>
                    </div>
                </div>
    ...
    <div class="carouselContainer md:pl-0">
        <div class="md:flex md:mx-[110px]">
            <div class="md:max-w-[552px]">
                ...
                <div class="flex mt-6 md:mt-8">
                    <button class="bg-[#0C24FB] text-white mr-6 h-[50px] w-[112px] md:w-[144px] font-switzer">Scopri di più</button>
                    <p class="text-[14px] self-center font-switzer font-medium leading-9">Lavora con noi</p>
                    <img src="{{ asset('assets/icons/arrow-right.svg') }}" alt="Arrow Right" class="w-[40px] h-[20px] self-center"/>
                </div>
            </div>
            <div class="mt-6 w-full h-[140px] md:h-[438px] md:ml-[65px] self-end bg-cover bg-right bg-[url('{{ asset('assets/images/ciatSede.jpg') }}')]">
            </div>
        </div>
    </div> <!-- Chi siamo -->
    ...
    <div class="carouselContainer">
        ...
        <div class="md:grid md:grid-cols-6 md:grid-rows-1 md:mt-2">
            <div class="mx-auto md:justify-self-stretch md:col-span-3 md:mx-0 w-[288px] h-[260px]  md:w-full md:h-[760px] flex-shrink-0
            bg-cover bg-center bg-[url('{{asset('/assets/images/showroom.jpg')}}')]"></div>
            <div class="flex flex-wrap justify-center md:justify-start md:gap-x-3 mt-6 md:mt-0 pb-4 md:col-span-3 md:ml-6 md:max-h-[920px] overflow-y-auto">
                @foreach($carousels[4]->products()->get()  as $product)
                    <div class="w-[130px] max-h-[286px] md:max-h-[440px] md:w-[282px] md:h-[440px] mr-1.5 md:mr-0 shrink-0 mb-2">
                        <div class="w-full h-[172px] md:h-[360px] flex bg-cover bg-center bg-[url('{{ asset($product->img) }}')]">
    ...
    <div class="carouselContainer md:px-[110px]">
        <div class="w-full md:w-auto h-[740px] md:h-[740px] md:py-0 mx-auto bg-interiorSecondary">
            <div class="md:grid md:grid-cols-6 md:grid-rows-1">
                <div class="mx-auto w-full h-[176px] md:h-[740px] md:col-start-4 md:col-span-3 md:row-start-1 bg-center bg-cover bg-[url('{{ asset('assets/images/collectionPlaceholder1.jpg') }}')]">
                </div>
                <div class="px-4 pt-2 md:max-w-[450px] md:h-[310px] md:m-auto md:my-[64px] md:px-2 md:py-0 md:col-start-1 md:col-span-3 md:row-start-1">
                    <p class="text-[12px] text-interiorMain font-din">INTERIOR DESIGN SERVICE</p>
                    <p class="text-[28px] md:text-[34px] text-interiorMain font-switzer font-bold leading-9 mt-1">Personal design shopper</p>
                    <p class="text-[13px] md:text-[14px] text-interiorMain font-switzer font-light leading-5 mt-1.5 md:mt-0.5">
    ...
    <div class="carouselContainer md:px-[110px]">
        ...
        <div class="md:grid md:grid-cols-3 md:grid-rows-1 md:gap-x-4">
            <div>
                <div class="mx-auto mt-6 w-full h-[140px] md:h-[320px]
                ...
                <p class="text-[18px] md:text-[24px] font-switzer font-medium leading-6 mt-1">
        ...
        <button class="bg-[#0C24FB] mt-6 text-white h-[50px] w-full md:w-[196px] md:mx-auto md:flex md:justify-center font-switzer">
            <p class="md:self-center">Leggi tutti gli articoli</p></button>
